...
import { addPrompt, getAllPrompts } from "./redux/promptSlice"
...
const UserInterface = ({ darkMode, setDarkMode }) => {
  const dispatch = useDispatch()
  const { prompt, loading } = useSelector((state) => state.prompt)
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(addPrompt(promptForm))
    setPromptForm({
      prompt: "",
      model: ""
    })
    setFinalResponse(prompt.answer)
  }

  return (
    <>
      {/* <div
        class={`px-8 pt-4 sm:ml-28 flex flex-col ${
          darkMode
            ? "bg-gray-900 text-white scroll-bar"
            : "bg-white text-gray-900"
        }`}>
        <div className="chevron-pattern rounded-xl p-3 mb-3 drop-shadow-lg animate__animated animate__fadeIn animate__slower">
          <div className="flex flex-row justify-self-center space-x-2 items-center transition-opacity duration-700 fade-in-out">
            <svg
              className={`${
            ...
            <BlurText
              ...
              animateBy="words"
              direction="top"
              className={`text-7xl font-rubik font-bold ${
                darkMode ? "text-black" : "text-white"
              } transition duration-300 ease-in-out`}
            />
        ...
        <div
          ...
          } rounded-lg drop-shadow-sm h-[61.4vh] overflow-y-auto overflow-x-hidden transition duration-100 ease-in-out`}>
          {loading ? (
            ...
            <>
              <div
                ...
                <span
                  className={`font-encode text-lg font-bold ${
        ...
        <section
          ...
          <div className="mx-auto py-4">
            <form onSubmit={handleSubmit}>
              <div
                class={`w-full mb-4 border rounded-lg drop-shadow-md ${
                  darkMode
                ...
                <div
                  ...
                  <textarea
                    ...
                    id="prompt"
                    rows="4"
                    class={`p-2 w-full px-2 text-sm border-0 not-focus-visible:ring-gray-50 focus:ring-gray-50 ${
                      darkMode
                        ? "bg-gray-800 text-white placeholder-gray-400"
                        : "bg-white text-gray-900"
                    } font-instrument text-lg tracking-wide`}
                ...
                <div
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPromptForm({ ...promptForm, model: e.target.value })
                    }
                    id="model"
                    className={`${
                      darkMode
                    ...
                    } border text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-[15vw] p-2.5 font-instrument tracking-wide`}>
                    <option value="">Select Model</option>
                    <option value="deepseek-r1">DeepSeek R1</option>
                    <option value="mistral">Mistral</option>
                    <option value="gemma2">Gemma2</option>
                  </select>
                  <div class="flex ps-0 space-x-1 rtl:space-x-reverse sm:ps-2 transition duration-300 ease-in-out">
                    {loading ? (
                      <motion.button
                        disabled
                        type="button"
                        whileHover={{ scale: 1.1 }}
                        class="text-white bg-teal hover:bg-teal-500 focus:ring-4 focus:ring-teal-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 inline-flex items-center font-rubik">
                        <svg
                          aria-hidden="true"
                          role="status"
                      ...
                      <motion.button
                        disabled={loading || promptForm.prompt === "" || promptForm.model === ""}
                        onKeyUp={(e) => e.key === "Enter" && handleSubmit()}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                        type="submit"
                        class="inline-flex items-center py-2.5 px-4 text-sm font-medium text-center text-white bg-teal rounded-lg focus:ring-4 focus:ring-teal-200 hover:bg-teal-500 font-rubik tracking-wide">
                        Submit
                      </motion.button>
                    )}
...
{/* <!-- ========== MAIN CONTENT ========== --> */}
<main class="md:ps-65 md:hs-overlay-minified:ps-13 transition-all duration-300 pb-4 h-screen flex flex-col bg-white dark:bg-neutral-800">
  {/* <!-- ========== HEADER ========== --> */}
    ...
    <div>
        ...
        <div class="pb-2 px-2">
          ...
          <div class="pt-2 flex justify-between items-center gap-x-1">
            ...
            <div class="flex items-center gap-x-1">
              ...
              {/* <!-- Send Button --> */}
              <button onClick={handleSubmit} type="button" class="inline-flex shrink-0 justify-center items-center size-8 text-sm font-medium rounded-lg text-white bg-cyan-700 hover:bg-cyan-600 disabled:opacity-50 disabled:pointer-events-none focus:outline-hidden focus:bg-cyan-600">
      ...
      <div class="mt-4">
        ...
        <div class="flex flex-wrap justify-center gap-1.5 sm:gap-2">
          ...
          <a class="py-1.5 px-2.5 inline-flex items-center gap-x-1.5 text-sm text-gray-800 bg-gray-100 hover:text-cyan-700 rounded-lg focus:outline-hidden focus:text-cyan-700 dark:bg-neutral-700 dark:text-neutral-200 dark:hover:text-neutral-400 dark:focus:text-neutral-400" href="#">
            <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m5 8 6 6"/><path d="m4 14 6-6 2-3"/><path d="M2 5h12"/><path d="M7 2h1"/><path d="m22 22-5-10-5 10"/><path d="M14 18h6"/></svg>
            Language
          </a>
        </div>
...
}
{
  /* {prompts.map((prompt, index) => (
            ...
            <div key={index}>
              ...
              <div> {new Date(prompt.date).toLocaleString("en-US", {
                          year: "numeric",
                          month: "long",
                          day: "numeric"
                        })}</div>
              <div><Markdown>{prompt.answer}</Markdown></div>
            </div>
